...
라인 트레이싱을 기반으로 모터 명령을 만들고, 명령기반으로 모터가 제어되는 것을 검증하였다. 

로봇 몸체에 해당 회로를 배선 하고, 동작하는 것을 검증할 예정이다
## 주요 데이터 흐름
**SW**
...
## 디버깅
하드웨어 구성 중 모터 드라이버 입력핀에서 개발 보드 GPIO OUTPUT핀으로 전류가 흘러 개발 보드 회로가 타는 상황 발생

멀티미터로 입력핀과 

해결방안

입력핀에서 전류가 흘러도 문제상황이 발생하지 않도록 저항으로 보호 회로를 구성해볼 예정이다.
